  ...
  return (
    <div className="relative min-h-screen" style={{ 
      ...
      <div className="container mx-auto flex justify-center items-center h-full">
        <div className="py-16">
          <div className={`shadow-md p-4 w-full ${artist?.theme.width}`}
            ...
            {artist && (artist?.pfp || artist?.name || artist?.desc || artist?.longDesc || isEditMode) && (
              <div className="mt-4 px-4 py-4" style={{ backgroundColor: artist?.theme.surface }}>
                <div className="px-12">
                  <div className="flex items-center">
                    ...
                    />
                    <div>
                      {isEditMode && <p className="text-xs text-teal-500 mb-1">Artist name</p>}
                      <h1 className="text-lg font-bold">
                        {isEditMode ? <input className="bg-transparent pl-1 focus:outline-none border border-teal-500 border" 
                          onBlur={() => updateDb()}
                      ...
                      {isEditMode && <p className="text-xs text-teal-500 mb-1 mt-2">Brief description</p>}
                      <div className="text-sm" style={{ color: artist?.theme.secondary }}>
                        {isEditMode ? <input className="bg-transparent pl-1 focus:outline-none border border-teal-500 mb-4" 
                          onBlur={() => updateDb()}
                ...
                {(artist.longDesc || isEditMode) && <div className="mt-4">
                ...
                {isEditMode && <>
                  ...
                  <div className="text-white font-bold flex">
                    <button className="px-2" onClick={() => setIsColorEditorOpen(!isColorEditorOpen)}
                      ...
                      {isColorEditorOpen ? "Close Color Editor ▲" : "Open Color Editor ▼"}
                    </button>
                  ...
                  {isColorEditorOpen && <div className="flex flex-wrap gap-x-8 gap-y-4 justify-center mt-4 mb-8">
                    <div>
            ...
            {isEditMode && <>
              ...
              <button className="text-white font-bold px-2" onClick={() => addOc()}
                style={{ backgroundColor: artist?.theme.primary, color: artist?.theme.surface }}>
                Create New OC Page +
              </button>
            </>}
            ...
            {artist && (artist.oc || isEditMode) && (
              <div className="mt-4">
                <div className="flex flex-col gap-4"> 
                  {artist.oc && Object.entries(artist.oc).map(([ocId, oc]) => (

                    // OC CARD
                    <div key={ocId} className="shadow-md p-4" style={{ backgroundColor: artist?.theme.surface }}>

                      {isEditMode && <div className="flex items-top justify-between">
                        <div className="flex-col w-1/4">
                          <SortingTagTipLabel text="OC sorting tag"/>
                        ...
                        <div className="flex items-center space-x-8 mb-4"> 
                          {oc.pfp && (
                            <a href={oc.pfp} target="_blank" rel="noopener noreferrer"> 
                              <img src={oc.pfp || pfp} className="w-36 h-36 object-cover"/> 
                            </a>
                          )}
                          <div> 

                            {isEditMode && <p className="text-xs text-teal-500 mb-1">Character name</p>}
                            
                            <h3 className="text-2xl font-semibold mb-2">
                              {isEditMode ? <input className="bg-transparent pl-1 focus:outline-none border border-teal-500" 
                                onBlur={() => updateDb()}
                            ...
                            {isEditMode && <p className="text-xs text-teal-500 mb-1">Brief description</p>}

                            <div style={{ color: artist?.theme.secondary }}>
                              {isEditMode ? <input className="bg-transparent pl-1 focus:outline-none border border-teal-500" 
                                onBlur={() => updateDb()}
                      ...
                      {(oc.longDesc || isEditMode) && <div className="mb-4">
                        {isEditMode && <p className="text-xs text-teal-500 mb-1">Detailed description text</p>}
                        {isEditMode ? <textarea className="bg-transparent pl-1 focus:outline-none border border-teal-500 w-full h-48" 
                          onBlur={() => updateDb()}
                      ...
                      {isEditMode && <div className="ml-4 w-full">
                        ...
                        {oc.links && <div className="flex items-center gap-2 w-full mr-4 pr-4 text-xs text-teal-500">
                          <div className="w-2/6"><SortingTagTipLabel text="OC sorting tag"/></div>
                      ...
                      {(oc.links) && Object.entries(oc.links).sort((a, b) => a[1].sortingTag.localeCompare(b[1].sortingTag)).map(([linkId, link]) => (
                        <div className="relative ml-4 text-md" key={linkId}>
                          {isEditMode ? 
                            ...
                            : <a href={link.url} target="_blank" rel="noopener noreferrer" style={{ color: artist?.theme.highlight }} className='underline'>
                          {link.label} </a>}
                        </div>
                      ))}
                      ...
                      <>
                        ...
                        <p className="text-xs text-teal-500 mt-2 mb-1">Additional references, fanarts, etc. - max. {Constants.max_gallery}</p>
                        <button className="text-white font-bold px-2 mb-4" onClick={() => addGalleryImage(ocId)}
                          style={{ backgroundColor: artist?.theme.primary, color: artist?.theme.surface }}>
                      ...
                      {oc.gallery && (
                        <div>
                          ...
                          <div className={isEditMode ? "flex-col space-y-4" : "flex flex-wrap gap-4 justify-start"}> 
                            {(oc.gallery) && Object.entries(oc.gallery).sort((a, b) => a[1].sortingTag.localeCompare(b[1].sortingTag)).map(([galleryId, gallery]) => (
                              <div className="relative" key={galleryId}>
                                {isEditMode ? <>
                                    <img
                                      src={gallery.img}
                                      className="w-auto h-[200px] object-cover mb-2" 
                                    />

                                    <div className="mb-1 ">
                                      <div className="flex gap-2 text-xs text-teal-500">
                                        <div className="w-1/6"><SortingTagTipLabel text="OC sorting tag"/></div>
                                        <p className="w-2/6">Description</p>
                                        <p className="w-1/6">Credit</p>
                                        <div className="w-2/6"><ImageUrlTipLabel text="URL"/></div>
                                        <button className="invisible"> <RiCloseLine size={16} /> </button>
                                      </div>
                                    </div>
                                    ...
                                    <div className="flex items-center mb-8 gap-2 w-full">
                                      ...
                                      <input className="w-1/6 bg-transparent pl-1 focus:outline-none border border-teal-500" 
                                        onBlur={() => updateDb()}
                                      ...
                                      <input className="w-2/6 bg-transparent pl-1 focus:outline-none border border-teal-500" 
                                        onBlur={() => updateDb()}
                                ...
                                <a href={gallery.img} target="_blank" rel="noopener noreferrer"> 
                                  ...
                                  />
                                  <span className="absolute bottom-0 left-0 bg-gray-800 text-white text-xs px-2 py-1 bg-opacity-75">
                                    {gallery.desc && <p>{gallery.desc}</p>}
                                    <p>By {gallery.credit ? gallery.credit : "unknown"}</p>
                                  </span>
                                </a>}
                              </div>
                            ))}
      ...
      <Dialog isOpen={dialogState === DialogState.ImageUrlTip} 
        ...
        <br/>
        <br/>
        <p className="font-bold">Method A - Recommended</p>
        Discord → Left click image → Click "Open in Browser" → Copy URL from browser
        <img
          src={imgTipA}
          className="my-2"/>
        <br/>
        <p className="font-bold">Method B - Quick & Low Resolution</p>
        Discord → Right click image → Click "Copy Link"
        <img
          src={imgTipB}
          className="my-2"/>
        <br/>
        <p className="font-bold">Method C - Other URLs</p>
        Any valid image URLs will work, so you can also rely on Google Drive, Imgur, social media, etc.
      </Dialog>
